...
export default function Expertises(props: any) {
  ...
  const stripePromise = loadStripe('pk_test_51Ms8N2DPRpXz3hgdPS0r0SuzXDLolvvarlf8MzF3khKt0xYwf0iS3Ic5w83dX7rVa6NTeS9cuFCFRklfKs7hpran00rKRnvo94');
    ...
    try {
      ...
      const res = await axios({
        ...
        data: {
          productId: the_id,
          // buyerId: id,
          buyerId: "d479b654-6a7a-40c2-b127-fe3869e242ec",
        },
        headers: {
  ...
  return (
    ...
    >
                ...
                {modalData?.data?.products.map((e) => (
                  <>
                    <div className=" w-full lg:max-w-full lg:flex">
                      {/* {e.name} */}
                      <Image src={e.imageUrl} className="flex-none w-auto h-full p-2 overflow-hidden text-center bg-cover rounded-3xl lg:rounded-t-none lg:rounded-l" width={100} height={100} title="Mountain" alt="alt" />
                      <div className="flex flex-col justify-between p-4 leading-normal  lg:rounded-b-none lg:rounded-r">
                        ...
                        <button className=" w-1/3 p-2 btn-semi-transparent" onClick={() => (handleBuy(e.id))} >Acheter</button>
      ...
      <div className="justify-center m-2 mb-4 md:flex md:flex-wrap md:flex-row h-80 md:mb-0 md:h-96">
        {data.map((e) => (
          <div key={Math.random().toString(36).substring(7)} className="relative items-center w-3/4 h-full p-10 mx-auto mt-2 mb-6 border-white md:w-1/4 gap-2 md:m-10 min-h-min bg-stone-900 bg-opacity-90 rounded-md">
